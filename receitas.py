...
janela.title('As Minhas Receitas')
# Largura * Altura
...
def login():
    ...
    #entry password
    txt_pw=Entry(janelalogin, width=25, show="*")
    txt_pw.place(x=100, y=100)
...
def registo():
    ...
    janelaregisto.geometry("300x300")
    ...
    #login button
    btn=Button(janelaregisto, text="Registe-se")
...
def add_receita():
    ...
    # Botões
    btn1=Button(janela_add, text='Guardar', fg='white', width=34, height=1, relief='ridge', command = add_receita, bg="#499dc0")
...
barra_Menu.add_command(label = "Login", command = login)
barra_Menu.add_command(label = "Registe-se", command = registo)

# Botões
btn1=Button(janela, text='Adicione uma nova Receita!', fg='white', width=35, height=3, relief='ridge', command = add_receita, bg="#499dc0")
btn1.pack(side=TOP)
btn2=Button(janela, text='Sopas', fg='black', width=7, height=3, relief='ridge', command = "noaction")
